...
do year=1,10
    ...
    do month=1,12
        ...
        do i=1,4
            ...
            do
                ...
                if (uperp(j10day) < -0.2814124 .or. &
                        uperp(j10day) >= -0.2814124+2.777613) cycle  ! WW
                t = mod(time, 48)
                if (t == 0) t = 48
    ...
    enddo
enddo
outfile = trim(outpath)//"/cltype_diurnal_DJFMA_WW.nc"
...
outfile = trim(outpath)//"/clsize_diurnal_DJFMA_WW.nc"
call check_nf90( nf90_create(outfile, NF90_NETCDF4, ncid) )
call check_nf90( nf90_def_dim(ncid, "X", 6, xid) )
call check_nf90( nf90_def_dim(ncid, "time", 48, timeid) )
call check_nf90( nf90_def_var(ncid, "X", NF90_DOUBLE, xid, xvid) )
